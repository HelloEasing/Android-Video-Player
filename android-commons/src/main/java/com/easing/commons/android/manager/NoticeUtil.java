...
public class NoticeUtil {
	...
	public static void showNotice(String title, String content, Intent action, int id) {
		String channelId = TextUtil.randomWithUpperCase();
		if (SystemUtil.getApiVersion() >= 26) {
			...
			NotificationManager manager = ServiceUtil.getNotificationManager(CommonApplication.ctx);
	...
	public static void cancelAll(Context ctx) {
		ServiceUtil.getNotificationManager(ctx).cancelAll();
	}
}
