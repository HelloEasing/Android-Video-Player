...
public class MultiDimenMap<T> {
    ...
    public static MultiDimenMap init(int dimen) {
        if (dimen <= 0)
            throw BizException.build("key dimen must be positive integer");
    ...
    public void put(T value, Object... keys) {
        if (keys.length != dimen)
            throw BizException.build("key dimen must equal to init length");
    ...
    public LinkedHashMap getMap(Object... keys) {
        LinkedHashMap map = coreMap;
        if (keys.length >= dimen)
            throw BizException.build("key dimen must be less than init length");
