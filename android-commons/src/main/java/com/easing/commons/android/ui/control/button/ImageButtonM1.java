...
public class ImageButtonM1 extends LinearLayout {
  ...
  private void customStyle(Context context, AttributeSet attrs, int style) {
    ...
    LinearLayout root = ViewManager.inflate(context, R.layout.layout_image_button_m01);
    ...
    View left = ViewManager.findView(root, R.id.left);
    View mid = ViewManager.findView(root, R.id.mid);
    View right = ViewManager.findView(root, R.id.right);
    if (left_padding != 0)
      ViewManager.size(left, left_padding, 0);
    if (mid_padding != 0)
      ViewManager.size(mid, mid_padding, 0);
    if (right_padding != 0)
      ViewManager.size(right, right_padding, 0);
