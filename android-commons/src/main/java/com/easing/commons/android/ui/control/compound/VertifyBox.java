...
public class VertifyBox extends LinearLayout {
    ...
    private void init(Context context, AttributeSet attrSet) {
        View root = ViewManager.inflate(context, R.layout.layout_validation_view);
        super.addView(root, new LayoutParams(ViewManager.MATCH_PARENT, ViewManager.MATCH_PARENT));
        input1 = ViewManager.findView(root, R.id.input1);
        input2 = ViewManager.findView(root, R.id.input2);
        button1 = ViewManager.findView(root, R.id.bt1);
        button2 = ViewManager.findView(root, R.id.bt2);
        input1.setHint("手机号码");
