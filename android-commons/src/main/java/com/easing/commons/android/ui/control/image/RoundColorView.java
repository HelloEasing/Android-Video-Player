...
public class RoundColorView extends View {
  ...
  private void init(Context context, AttributeSet attrSet) {
    ...
    cirlePaint.setColor(ColorUtil.LIGHT_BLUE);

    textPaint = new Paint();
    textPaint.setAntiAlias(true);
    textPaint.setColor(ColorUtil.WHITE);
    textPaint.setTextSize(DimenUtil.toPx(ctx, 14));
  }
  ...
  public void set(Integer circleColor, Integer textColor, String text, Integer fontSize) {
    ...
    if (text != null)
      this.text = text;
    if (fontSize != null)
      textPaint.setTextSize(DimenUtil.toPx(ctx, fontSize));
    invalidate();
  }
}
