...
public class CaptchaView extends AppCompatImageView {
	...
	private void randomCode() {
		...
		for (int i = 0; i < CODE_LENGTH; i++)
			builder.append(CHARS[MathUtil.randomInt(0, CHARS.length - 1)]);
	...
	private void randomStyle(Paint paint) {
		...
		paint.setTextSkewX(MathUtil.randomInt(-10, 10) / 10f);
	}
	
	//随机生成位置偏移
	private void randomPadding() {
		paddingLeft = paddingLeft + PADDING_LEFT_BASE + MathUtil.randomInt(5, PADDING_LEFT_EXTRA);
		paddingTop = PADDING_TOP_BASE + MathUtil.randomInt(2, PADDING_TOP_EXTRA);
	}
	
	//随机生成干扰横线
	private void randomLine(Canvas canvas, Paint paint) {
		int color = MathUtil.randomInt(0xBB000000, 0xBBFFFFFF);
		int startX = MathUtil.randomInt(0, BITMAP_WIDTH);
		int startY = MathUtil.randomInt(0, BITMAP_HEIGHT);
		int endX = MathUtil.randomInt(0, BITMAP_WIDTH);
		int endY = MathUtil.randomInt(0, BITMAP_HEIGHT);
		int width = MathUtil.randomInt(1, LINE_WIDTH) + 1;
