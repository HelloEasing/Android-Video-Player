...
public class TabItem extends LinearLayout {
    ...
    private void init(Context context, AttributeSet attrSet) {
        ...
        fontSize = attrs.getDimension(R.styleable.TabItem_fontSize, DimenUtil.toPx(context, 14));
        normalColor = attrs.getColor(R.styleable.TabItem_normalColor, ColorUtil.LIGHT_GREY);
        activeColor = attrs.getColor(R.styleable.TabItem_activeColor, ColorUtil.LIGHT_BLUE);

        //设置Item布局和内边距
        super.setOrientation(LinearLayout.VERTICAL);
        super.setLayoutParams(new LayoutParams(0, ViewManager.MATCH_PARENT, 1));
        super.setPadding(0, DimenUtil.toPx(context, 5), 0, DimenUtil.toPx(context, 5));
        //添加图片
        iv = new ImageView(context);
        iv.setImageResource(icon);
        iv.setColorFilter(normalColor);
        iv.setScaleType(ImageView.ScaleType.CENTER_INSIDE);
        super.addView(iv, new LayoutParams(ViewManager.MATCH_PARENT, 0, 1));
        //添加文字
        ...
        super.addView(tv, new LayoutParams(ViewManager.MATCH_PARENT, ViewManager.WRAP_CONTENT, 0));
        //设置图片文字间距
        LayoutParams tvLayoutParams = (LayoutParams) tv.getLayoutParams();
        tvLayoutParams.setMargins(0, DimenUtil.toPx(context, 2), 0, 0);
