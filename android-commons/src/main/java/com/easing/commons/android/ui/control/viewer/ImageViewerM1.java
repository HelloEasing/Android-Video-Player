...
public class ImageViewerM1 extends LinearLayout {
  ...
  private void customStyle(Context context, AttributeSet attrs) {
    View dispaly_pane = ViewManager.inflate(context, R.layout.layout_gallery_m01);
    ...
    SCREEN_WIDTH = ViewManager.getScreenSize((Activity) this.getContext()).w;
    PADDER_WIDTH = SCREEN_WIDTH / 2 - IMAGE_WIDTH / 2;
    ViewManager.size(scroller, ViewManager.MATCH_PARENT, IMAGE_WIDTH, 0f);
    ViewManager.size(dispaly_pane, ViewManager.MATCH_PARENT, 0, 1f);
    ViewManager.size(scroller.getLeftPadder(), PADDER_WIDTH, 1, 0);
    ViewManager.size(scroller.getRightPadder(), PADDER_WIDTH, 1, 0);
    
    ViewManager.setBackground(scroller, R.drawable.color_black_20);
    ...
    scroller.setViewMapper((bitmap) ->
    {
      ...
      ViewManager.setBackground(iv, R.drawable.container_back_m17);
      ViewManager.size(iv, IMAGE_WIDTH, IMAGE_WIDTH, 0);
  ...
  private int computeSelectedItem() {
    ...
    for (int i = 0; i < scroller.getViews().size(); i++) {
      ...
      Pos pos = ViewManager.getPositionOnScreen(v);
      int x1 = pos.x;
