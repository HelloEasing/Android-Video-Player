...
public class ColorUtil {
    ...
    public static int reverseColor(int color, int alpha) {
        String str = MathUtil.hexToString(color);
        if (str.length() != 8)
            throw new RuntimeException("need a color value of 8 chars");
        
        int r = 255 - MathUtil.hexStringToInt(str.substring(2, 4));
        int g = 255 - MathUtil.hexStringToInt(str.substring(4, 6));
        int b = 255 - MathUtil.hexStringToInt(str.substring(6, 8));
    ...
    public static int getColor(View v, int colorId) {
        return v.getContext().getResources().getColor(colorId);
    }
    
    public static int randomColor(){
        return MathUtil.randomInt(0xFF000000,0xFFFFFFFF);
    }
}
